...
class MainSelectorComponent(ModeSelectorComponent):
    ...
    def __init__(self, matrix, top_buttons, side_buttons, config_button, parent):
        ...
        #CC self._stepseq = StepSequencerComponent(self, self._matrix, self._side_buttons, self._nav_buttons)
        self.init_session()
    ...
    def disconnect(self):
        ...
        self.buttonsToSwitchForMixer = None
    ...
    def update(self):
        ...
        if self.is_enabled():
            ...
            if (self._mode_index == 0):
                ...
                #CC self.setup_step_sequencer((not as_active))
                ...
                self.setup_mixer((not as_active))
                self.setup_user1(True, True, True) 
                #CC self.setup_step_sequencer(as_active)
            elif (self._mode_index == 3):
                #CC self.setup_step_sequencer((not as_active))
    ...
    def init_session(self):
        #log("init_session")        
        self._session.set_stop_track_clip_value(AMBER_FULL)
        for scene_index in range(self._matrix.height()):
            ...
            for track_index in range(self._matrix.width()):
                ...
                clip_slot.set_stopped_value(AMBER_HALF)
