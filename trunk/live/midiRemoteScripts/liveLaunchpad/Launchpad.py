...
class Launchpad(ControlSurface):
	...
	def __init__(self, c_instance):
		...
		self.log("LaunchPad85 Loaded !")

	def disconnect(self):
		self.log("Launchpad::disconnect")
		self._suppress_send_midi = True
		for control in self.controls:
			if isinstance(control, ConfigurableButtonElement):
	...
	def _set_session_highlight(self, track_offset, scene_offset, width, height, include_return_tracks):
		self.log("Launchpad::_set_session_highlight (" + str(track_offset) + ", " + str(scene_offset) + ", " + str(width) + ", " + str(height) + ")")
	...
	def log(self, message):
		self._my_c_instance.log_message(message)
