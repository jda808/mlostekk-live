...
class PreciseButtonSliderElement(ButtonSliderElement):
	...
	" SET THE VALUE MAP "
	def set_value_map(self, themap):
		assert isinstance(themap, (tuple, type(None)))
		assert (len(themap) == len(self._buttons))
		self._value_map = themap
		...
		if ((not self._disabled) and ((value != 0) or (not sender.is_momentary()))):
			...
			for notification in self._value_notifications:
				...
				if notification["Identify"]:
					callback(midi_value, self) #@UndefinedVariable
				else:
