...
class SubSelectorComponent(ModeSelectorComponent):
    ...
    def __init__(self, matrix, side_buttons, session, parent):
        ...
        for column in range(matrix.width()):
            ...
            self._sliders[-1].name = ("Button_Slider_" + str(index))
    ...
    def set_mode(self, mode):
        #log("SubSelectorComponent::set_mode (mode:" + str(mode) + ")")
    ...
    """ update """
    def update(self):
        #log("SubSelectorComponent::update")
        ...
        if self.is_enabled():
            ...
            if (self._mode_index == -1):
                ...
                self.setup_volume_mode()
            elif (self._mode_index == 1):
                self.setup_pan_mode()
            elif (self._mode_index == 2):
                self.setup_send1_mode()
            elif (self._mode_index == 3):
                self.setup_send2_mode()
            else:
                assert False
    ...
    def setup_mixer_overview(self):
        #log("SubSelectorComponent::setup_mixer_overview")
        trkon_index = 5
        #stop_buttons = []
        for track in range(self._matrix.width()):
        ...
        for track in range(self._numTracks):
            if (TrackFinder.getTrackIndexArray().count(track) > 0):
                matrixIndex += 1
                stop_buttons.append(self._matrix.get_button(matrixIndex, 7))
            else:
                stop_buttons.append(None)
                
        self._session.set_stop_track_clip_buttons(tuple(stop_buttons))
        self._session.set_stop_all_clips_button(self._side_buttons[3])
        self._mixer.set_global_buttons(self._side_buttons[2], self._side_buttons[1])


    """ setup volume mode """
    def setup_volume_mode(self):
        log("SubSelectorComponent::setup_volume_mode")
        for track in range(self._matrix.width()):
            #strip = self._mixer.channel_strip(track)
            strip = self._mixer.channel_strip(TrackFinder.getTrackIndexArray()[track])
            strip.set_default_buttons(None, None, None, None)
    ...
    """ setup pan mode """
    def setup_pan_mode(self):
        log("SubSelectorComponent::setup_pan_mode")
        for track in range(self._matrix.width()):
            #strip = self._mixer.channel_strip(track)
            strip = self._mixer.channel_strip(TrackFinder.getTrackIndexArray()[track])
            strip.set_default_buttons(None, None, None, None)
    ...
    """ setup send1 mode """
    def setup_send1_mode(self):
        log("SubSelectorComponent::setup_send1_mode")
        for track in range(self._matrix.width()):
            #strip = self._mixer.channel_strip(track)
            strip = self._mixer.channel_strip(TrackFinder.getTrackIndexArray()[track])
            strip.set_default_buttons(None, None, None, None)
    ...
    """ setup send2 mode """
    def setup_send2_mode(self):
        log("SubSelectorComponent::setup_send2_mode")
        for track in range(self._matrix.width()):
            #strip = self._mixer.channel_strip(track)
            strip = self._mixer.channel_strip(TrackFinder.getTrackIndexArray()[track])
            strip.set_default_buttons(None, None, None, None)
